...
describe('format utils', () => {
  ...
  describe('formatPlayerTitle', () => {
    ...
    it('formats series with episode info', () => {
      expect(
        formatPlayerTitle({ name: 'Breaking Bad' }, { season: 1, episode: 1, title: 'Pilot' })
      ).toBe('Breaking Bad S1E1: Pilot');
    });

    it('formats series without episode title', () => {
      expect(formatPlayerTitle({ name: 'The Office' }, { season: 2, episode: 3 })).toBe(
        'The Office S2E3'
      );
    });
  ...
  describe('formatReleaseInfo', () => {
    it('prefers meta releaseInfo when present', () => {
      expect(formatReleaseInfo({ releaseInfo: '2023-2024', released: '2023-01-01' })).toBe(
        '2023-2024'
      );
    });
  ...
  describe('formatDescription', () => {
    it('prefers video overview', () => {
      expect(
        formatDescription({ description: 'Meta description' }, {
    ...
    it('falls back to meta description', () => {
      expect(formatDescription({ description: 'A great movie about...' })).toBe(
        'A great movie about...'
      );
    });
