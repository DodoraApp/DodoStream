...
export function useGithubReleaseStatus(params: {
  ...
  const checkNow = useCallback(async () => {
    ...
    const version = tagName ? (extractSemverFromText(tagName) ?? normalizeVersion(tagName)) : '';

    const updateAvailable =
      installedVersion && version ? areVersionsDifferent(installedVersion, version) : null;
