...
export const usePlayerSeek = ({
  ...
  const handleSeekStart = useCallback(() => {
    debug('seekStart', { currentTime: currentTimeRef.current });
    onInteraction?.();
    setIsSeeking(true);
    setSeekTime(currentTimeRef.current);
    lastSeekTimeRef.current = currentTimeRef.current;

    // Remember if we need to resume after seeking
    wasPlayingBeforeSeekRef.current = !pausedRef.current;
  ...
  const handleSeekChange = useCallback(
    (value: number) => {
      ...
      seekDebounceTimeoutRef.current = setTimeout(() => {
        ...
        const commitValue =
          max > 0 ? Math.min(lastSeekTimeRef.current, max) : lastSeekTimeRef.current;
