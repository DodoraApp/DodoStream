...
export function useSearchCatalogs(query: string, enabled: boolean = true) {
  ...
  const results = useQueries({
    queries: searchableCatalogs.map(
      ({ manifestUrl, catalogType, catalogId, catalogName, addonName }) => ({
        queryKey: [
 ...
 */
function addonSupportsContent(addon: InstalledAddon, type: ContentType, id: string): boolean {
  const { manifest } = addon;
 ...
 */
const heroSelectionKey = (
  sources: { manifestUrl: string; type: string; catalogId: string }[],
  itemCount: number
) =>
  [
    'hero-selection',
    ...sources.map((s) => `${s.manifestUrl}-${s.type}-${s.catalogId}`),
    itemCount,
  ] as const;
...
export function useHeroCatalogContent(
  ...
  const catalogResults = useQueries({
    queries: sources.map((source) => ({
      ...
      queryFn: () =>
        fetchCatalogWithPagination(source.manifestUrl, source.type, source.catalogId, 0),
