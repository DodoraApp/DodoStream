...
describe('Simkl Sync Service - Comprehensive E2E', () => {
  ...
  describe('Import Flow', () => {
    ...
    it('protects movies and shows from being deleted by anime category', async () => {
        ...
        mockGetAllItems.mockImplementation((_token, type, _dateFrom, extended) => {
            ...
            if (type === 'movies') return Promise.resolve({ movies: [{ movie: { ids: { imdb: 'tt_movie' } } }] });
            if (type === 'shows') return Promise.resolve({ shows: [{ show: { ids: { imdb: 'tt_show' } } }] });
    ...
    it('imports watching/hold/plantowatch to My List and watching/completed/hold to history', async () => {
        mockGetAllItems.mockImplementation((_token, type) => {
            if (type === 'movies') {
                return Promise.resolve({
                    movies: [
                        { movie: { ids: { imdb: 'tt_watching' } }, status: 'watching', last_watched_at: '2026-01-01T00:00:00Z' },
                        { movie: { ids: { imdb: 'tt_hold' } }, status: 'hold', last_watched_at: '2026-01-01T00:00:00Z' },
                        { movie: { ids: { imdb: 'tt_plantowatch' } }, status: 'plantowatch' },
                        { movie: { ids: { imdb: 'tt_completed' } }, status: 'completed', last_watched_at: '2026-01-01T00:00:00Z' },
        ...
        expect(mockAddToMyList).toHaveBeenCalledWith('profile-1', 'tt_plantowatch', 'movie', undefined);
        expect(mockAddToMyList).not.toHaveBeenCalledWith('profile-1', 'tt_completed', 'movie', expect.anything());

        // History upserts: watching, hold, completed
        expect(mockUpsertWatchProgress).toHaveBeenCalledWith(expect.objectContaining({ metaId: 'tt_watching' }));
        expect(mockUpsertWatchProgress).toHaveBeenCalledWith(expect.objectContaining({ metaId: 'tt_hold' }));
        expect(mockUpsertWatchProgress).toHaveBeenCalledWith(expect.objectContaining({ metaId: 'tt_completed' }));
        expect(mockUpsertWatchProgress).not.toHaveBeenCalledWith(expect.objectContaining({ metaId: 'tt_plantowatch' }));
    ...
    it('aborts removal sync if any getAllItems call fails (Empty Response Safety)', async () => {
        mockListWatchHistory.mockResolvedValue([
            { id: 'tt_item_1', type: 'movie', source: 'simkl' }
        ]);
    ...
    it('prevents concurrent imports for the same profile', async () => {
        mockGetActivities.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve({}), 100)));
    ...
    it('correctly maps various anime types (OVA, ONA, Special)', async () => {
        ...
        expect(mockUpsertWatchProgress).toHaveBeenCalledWith(expect.objectContaining({ metaId: 'tt_ova', type: 'series' }));
        expect(mockUpsertWatchProgress).toHaveBeenCalledWith(expect.objectContaining({ metaId: 'tt_ona', type: 'series' }));
        expect(mockUpsertWatchProgress).toHaveBeenCalledWith(expect.objectContaining({ metaId: 'tt_special', type: 'series' }));
        expect(mockUpsertWatchProgress).toHaveBeenCalledWith(expect.objectContaining({ metaId: 'tt_music', type: 'series' }));
    });
  });
  ...
  describe('Export Flow', () => {
    it('exports completed items and watchlist items', async () => {
      ...
      mockListExportableMyList.mockResolvedValue([
          { id: 'tt_exp_watchlist', type: 'series' }
      ]);
      ...
      expect(mockPostHistory).toHaveBeenCalledWith('token', expect.objectContaining({
          movies: [expect.objectContaining({ ids: { imdb: 'tt_exp_history' } })]
      }));
      expect(mockPostWatchlist).toHaveBeenCalledWith('token', expect.objectContaining({
          shows: [expect.objectContaining({ ids: { imdb: 'tt_exp_watchlist' }, to: 'plantowatch' })]
      }));
    });

    it('exports removals from sync queue', async () => {
        mockListSyncQueueForProvider.mockResolvedValue([
            { id: 1, metaId: 'tt_remove', type: 'movie', action: 'remove_history', createdAt: Date.now() }
        ]);
        ...
        expect(mockRemoveFromHistory).toHaveBeenCalledWith('token', expect.objectContaining({
